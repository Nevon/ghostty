...
pub const Dirty = packed struct {
    ...
    /// Set when search matches change, requiring full screen redraw
    /// because matches can span multiple lines.
    search_matches: bool = false,
};
...
pub const SearchState = struct {
    /// The current search term. May be owned by this struct or shared with
    /// another SearchState (e.g., in a cloned screen).
    needle: []const u8,
    ...
    /// Whether this SearchState owns the needle and should free it on deinit.
...
pub fn deinit(self: *Screen) void {
    ...
    // Clean up search state if it exists
